...
public class JSONgenerator {
	...
	public ArrayList<String> generateMiniNet(int[][][] edges, int max){
       ...
       //Map<Pair, Integer> m=new HashMap<Pair, Integer>();
		...
		for(int i=0; i<max; i++){
			...
			for(int j=0; j<edges[0].length; j++){
				for(int k=0; k<edges[0][0].length; k++){
					...
					if(edges[i][j][k]>0){
						String sName=""+j+"_"+i;
						String tName=""+k+"_"+(i+1);
						if(!nameMap.containsKey(sName)){
						    names.add(new Name(sName));
						    nameMap.put(sName, index);
						    index++;
						}
						
						if(!nameMap.containsKey(tName)){
							names.add(new Name(tName));
							nameMap.put(tName, index);
							index++;
						}
						
						links.add(new Link(nameMap.get(sName), nameMap.get(tName), edges[i][j][k]));
					}
		...
		jsons.add((new JSONArray(names)).toString());
		jsons.add((new JSONArray(links)).toString());
	...
	class Link{
		...
		public Link(int s, int t, double v){
			source=s;
			target=t;
			value=v;
		}
